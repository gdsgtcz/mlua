...
#ifndef hhl_mlua_atomdict_ad
#define hhl_mlua_atomdict_ad
...
_find_slot(lua_State *L , struct atomdict *ad) {
	...
	for (j=0;j<2;j++) {
		for (i=0;i<MAX_HANDLE;i++) {
            if (GetG::getInstance()->G[i] == NULL) {
				if (__sync_bool_compare_and_swap(&(GetG::getInstance()->G[i]), NULL , ad)) {
					return i;
				}
			}
		}
...
_new_handle(lua_State * L) {
	...
	const char * typename_ = NULL;
	if (lua_gettop(L) > 1) {
		typename_ = luaL_checkstring(L,2);
	}
	...
	int r = atomdict_init(ad, slot, typename_);
	if (r != 0) {
...
_delete_handle(lua_State *L) {
    ...
    struct atomdict *ad = GetG::getInstance()->G[idx];
	GetG::getInstance()->G[idx] = NULL;
	__sync_synchronize();
	atomdict_delete(ad);
	return 0;
...
//#define _update_object(L,object)  _update_object_(L,object->barrier,object)
...
static void
_gen_keyindex_from_meta(lua_State *L, struct atomdict *ad, const char *typename_) {
	lua_getfield(L,-1,"type");
	// stack now : ... , metatable , typecache 
	lua_getfield(L, -1 , typename_);
	if (lua_isnil(L,-1)) {
		lua_pop(L,-1);
		_gen_keyindex(L, ad);
		lua_pushvalue(L,-1);
		lua_setfield(L,-3,typename_);
...
_new_object(lua_State *L) {
	struct barrier * b = (struct barrier *)lua_touserdata(L, lua_upvalueindex(1));
	int index = luaL_checkinteger(L,1);
    struct atomdict * ad = GetG::getInstance()->G[index];
	if (ad == NULL) {
		return luaL_error(L, "Atomdict binding a null handle");
	}

	struct ad_object *object = (struct ad_object *)lua_newuserdata(L,sizeof(struct ad_object));
	object->ad = ad;
	object->barrier = b;
	object->version = -1;
	...
	const char * typename_ = (const char *)atomdict_typename(ad);

	if (typename_) {
		_gen_keyindex_from_meta(L, ad, typename_);
	} else {
		_gen_keyindex(L, ad);
	}
...
_name_object(lua_State *L) {
	struct ad_object *object = (struct ad_object *)lua_touserdata(L,1);
	const char * typename_ = atomdict_typename(object->ad);
	if (typename_ == NULL) {
...
_get_object(lua_State *L) {
	...
	if (index > 0) {
        ...
        //guodi changed
        //get the last data
        //float v = atomdict_get_number(object->ad, atomdict_grab(object->ad), index-1);
		lua_pushnumber(L,v);
	} else {
        const char * v = atomdict_get_string(object->ad, object->col, -(index+1));
        //guodi changed
        //get the last data
        //const char * v = atomdict_get_string(object->ad, atomdict_grab(object->ad), -(index+1));
		lua_pushstring(L,v);
	}
	return 1;
}
...
_dump_object(lua_State *L) {
	struct ad_object * obj = (struct ad_object *)lua_touserdata(L,1);
...
luaopen_atomdict(lua_State *L) {
	...
	struct barrier * b = (struct barrier *)lua_newuserdata(L, sizeof(struct barrier));
	b->version = 0;
	b->object = 0;
